				...
				<div class="col-lg-12 m-b30">
					<div class="widget-box">
						...
						<div class="widget-inner">
                            <form class="edit-profile m-b30" method="POST" action="{{route('course.update', $course->id)}}" enctype="multipart/form-data">
                                @method('PUT')
                                @csrf
								<div class="row">
									<div class="col-12">
									...
									<div class="form-group col-6">
										...
										<div>
											<input class="form-control" type="text" name="title" value="{{old('title', $course->title)}}"/>
										</div>
									</div>
									<div class="form-group col-6">
										<label class="col-form-label">Course slug</label>
										<div>
                                            <input class="form-control" value="{{old('slug', $course->slug)}}" type="text" name="slug"  />
										</div>
									</div>
                                    ...
                                    <div class="form-group col-6">
										...
										<div>
                                            <input class="form-control" value="{{old('fee', $course->fee)}}" type="text" name="fee" placeholder="$"/>
										</div>
									</div>
                                      ...
                                      <div class="form-group col-6">
										...
										<div>
                                            <input class="form-control" value="{{old('language', $course->language)}}" type="text" name="language"/>
										</div>
									</div>
                                    ...
                                    <select name="assessments" id="assessments" required>
                                        <option value="yes" {{$course->assessments == 'yes' ? 'selected' : ' '}}>Yes</option>
                                        <option value="no"  {{$course->assessments == 'no' ? 'selected' : ' '}}>No</option>
                                    </select>
                                    </div>
                                        ...
                                        <div class="form-group col-6">
                                            ...
                                            <select name="skill_level" id="skill_lavel" required>
                                                <option value="beginner"            {{$course->skill_level == 'beginner' ? 'selected' : ' '}}>Beginner(A1)</option>
                                                <option value="elementary"          {{$course->skill_level == 'elementary' ? 'selected' : ' '}}>Elementary(A2)</option>
                                                <option value="intermediate"        {{$course->skill_level == 'intermediate' ? 'selected' : ' '}}>Intermediate(B1)</option>
                                                <option value="upper-intermediate"  {{$course->skill_level == 'upper-intermediate' ? 'selected' : ' '}}>Upper-Intermediate(B2)</option>
                                                <option value="advanced"            {{$course->skill_level == 'advanced' ? 'selected' : ' '}}>Avanced(C1)</option>
                                                <option value="proficiency"         {{$course->skill_level == 'proficiency' ? 'selected' : ' '}}>Proficiency(C2)</option>
                                            </select>
                                            </div>
                                            ...
                                            <label class="col-form-label">Select Category</label>
                                                ...
                                                @foreach (\App\Models\Category::all() as $category )
                                                    <option value="{{$category->id}}"
                                                        {{$category->id  == $course->category_id ? 'selected': ''}}>{{ucwords($category->name)}}</option>
                                                    @endforeach
                                                </select>
                                        </div>
									...
									<div class="form-group col-12">
										...
										<div>
											<textarea class="form-control" value="{{old('description',$course->description)}}" name="description" cols="50" rows="2">{{old('description',$course->description)}}</textarea>
										</div>
									</div>
                                    ...
                                    <div class="form-group col-12">
										...
										<div>
                                            <textarea class="form-control" value="{{old('certification',$course->certification)}}" name="certification" cols="50" rows="10">{{old('certification', $course->certification)}}</textarea>
										</div>
									</div>
                                      <div class="form-group col-12">
